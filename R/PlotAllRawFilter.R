...
#'  @param corvalue.mean mean of correlation values
#'  @param peakgroup.num differentiate from different samples
#'  @param mcicutoff the MCI score cutoff

PlotAllRawFilter <- function(ms1, ms2, feature, corvalue, ms2.maxi, ms2.maxi.cutoff, dpvalue, corvalue.mean, peakgroup.num, mcicutoff){
  dir.create("EIC and spectra after filter") 
  setwd("EIC and spectra after filter")
  for(i in 1:length(ms1)){
    
    if(dpvalue[i] < mcicutoff | corvalue.mean < mcicutoff) next
