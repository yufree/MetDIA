#' RefineReuslt: Refine output result
#' RefineReuslt: Add detailed information, such as KEGG, HMDB, to the result
#' @param result.raw raw result of MS1 and SWATH ID
#' @param inf.detail detail inforamtion containing metabolite name, lab ID, KEGG ID, HMDB ID
#' @param name.sample sample name

RefineReuslt <- function(result.raw, inf.detail, name.sample){
  ...
  idx.area.begin <- match(name.sample[1], colnames(result.raw))
  idx.area.end <- match(name.sample[length(name.sample)], colnames(result.raw))
